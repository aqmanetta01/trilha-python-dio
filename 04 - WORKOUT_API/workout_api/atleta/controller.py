...
async def query_all(db_session: DatabaseDependency) -> List[AtletaResponse]:
    atletas = (await db_session.execute(select(AtletaModel))).scalars().all()
    return [AtletaResponse(nome=atleta.nome, centro_treinamento=atleta.centro_treinamento, categoria=atleta.categoria) for atleta in atletas]
